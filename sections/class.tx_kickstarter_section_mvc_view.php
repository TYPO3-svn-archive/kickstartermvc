...
class tx_kickstarter_section_mvc_view extends tx_kickstarter_section_mvc_base {
	var $sectionID = 'mvcview';
	...
	function render_wizard() {
		...
		if ($action[0]=='edit')	{
			...
			$lines = $this->catHeaderLines($lines, $this->sectionID, $this->wizard->options[$this->sectionID], '<strong>Edit View #'.$action[1].'</strong>', $action[1]);
			$piConf   = $this->wizard->wizArray[$this->sectionID][$action[1]];
			$ffPrefix = '['.$this->sectionID.']['.$action[1].']';

				// Enter title of the plugin
			$subContent='<strong>Enter a title for the view:</strong><br />'.
				$this->renderStringBox($ffPrefix.'[title]',$piConf['title']);
			$lines[]='<tr'.$this->bgCol(3).'><td>'.$this->fw($subContent).'</td></tr>';

            $subContent='<strong>Enter a short description for the view:</strong><br />'.
            ...
            $lines[] = '<tr><td><strong>Free name for view class</strong></td></tr>';
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter__mvc_ex/sections/class.tx_kickstarter_section_mvc_view.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/kickstarter__mvc_ex/sections/class.tx_kickstarter_section_mvc_view.php']);
}
